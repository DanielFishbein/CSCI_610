...
function setUpTextures(){
    ...
    volcano_texture = gl.createTexture();
    
    gl.bindTexture(gl.TEXTURE_2D, volcano_texture);
    
    // load the actual image
    var volcano_image = document.getElementById ('volcano-texture')
    volcano_image.crossOrigin = "";
    volcano_image.onload = () => {
    ...
    // load the texture data
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, volcano_image.width, volcano_image.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, volcano_image);
    ...
    // // load the actual image
    // var checker_board_image = document.getElementById ('checker-board-texture')
    // checker_board_image.crossOrigin = "";
    // checker_board_image.onload = () => {
...
function drawCurrentShape () {
    ...
    if(curTexture == "globe"){
      gl.uniform1i(program.index_location, 0)
      gl.activeTexture (gl.TEXTURE0);
      gl.bindTexture (gl.TEXTURE_2D, worldTexture);
      gl.uniform1i (program.uTheTexture, 0);
    }
    else if(curTexture == "myimage"){
      gl.uniform1i(program.index_location, 1)
      gl.activeTexture (gl.TEXTURE1);
      gl.bindTexture (gl.TEXTURE_2D, volcano_texture);
      gl.uniform1i (program.the_other_texture, 1);
    }
    else if (curTexture == "proc"){
      gl.uniform1i(program.index_location, 2)
...
function initProgram (vertexid, fragmentid) {
  ...
  program.the_other_texture = gl.getUniformLocation(program, 'theTexture_the_second');
  program.index_location = gl.getUniformLocation(program, "texture_index");
  // program.procedural_texture = gl.getUniformLocation(program, "pro_texture");
  return program;
}
